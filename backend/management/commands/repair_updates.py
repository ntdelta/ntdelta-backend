...
def run_seed(self, mode):
    ...
    with open(
        "C:\\Users\\Administrator\\Downloads\\rawdogjson\\rawdogjson",
        encoding="utf-8",
    ) as json_file:
                    ...
                    for update_name, update_data in updates.items():
                        ...
                        if "updateInfo" in update_data.keys():
                            win_update = WindowsUpdate.objects.filter(
                                release_version=update_data["updateInfo"][
                                    "releaseVersion"
                                ],
                            )
                            if len(win_update) > 0:
                                ...
                                print(
                                    update_data["updateInfo"]["releaseVersion"],
                                )
                                if not instance.windows_updates.filter(
                                    id=win_update.id,
                                ).exists():
                                    print("lets add")
                            ...
                            win_update = WindowsUpdate.objects.filter(
                                release_date=update_data["windowsVersionInfo"][
                                    "releaseDate"
                                ],
                            ...
                            if len(win_update) > 0:
                                win_update = win_update.first()
                                print(
                                    update_data["windowsVersionInfo"]["releaseDate"],
                                )
                                print("BASE")
                                if not instance.windows_updates.filter(
                                    id=win_update.id,
