...
from application.ntdelta.backend.ghidra_integration_lib import get_dll_windindex_content
from application.ntdelta.backend.ghidra_integration_lib import open_json_file
...
def seed_data():
    ...
    extension = ".txt"
    matching_files = find_files_with_extension(
        ...
        for key, value in json_data.items():
            if "fileInfo" in value.keys() and "sha256" in value["fileInfo"].keys():
                ...
                for win_ver_key, win_ver_value in value["windowsVersions"].items():
                    (
                        windows_version_obj,
                        windows_version_obj_created,
                    ) = WindowsVersion.objects.get_or_create(
                        name=win_ver_key,
                    )
                    ...
                    for win_upd_key in win_ver_value:
                        ...
                        if "updateInfo" in win_upd_value.keys():
                            (
                                ...
                                release_version=win_upd_value["updateInfo"][
                                    "releaseVersion"
                                ],
                ...
                (
                    dll_instance_obj,
                    dll_instance_obj_created,
                ) = DLLInstance.objects.get_or_create(
                    dll=dll_obj,
                ...
                [
                    dll_instance_obj.windows_updates.add(
                        j,
                    )
                    for j in windows_updates_dll_present
                ]
                ...
                for function_name in json_functions["ghidra" + key]:
                    ...
                    with open(function_decomp_path, encoding="utf-8") as file:
