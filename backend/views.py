...
from backend.models import DLL
from backend.models import DLLInstance
from backend.models import Function
from backend.models import WindowsUpdate
from backend.models import WindowsVersion
    ...
    if request.method == "POST":
        ...
        if auth_header and auth_header.startswith("Basic "):
            ...
            if username == settings.API_USERNAME and password == settings.API_PASSWORD:
                ...
                if "builds" in post_data["update_info"].keys():
                    ...
                    for windows_version, windows_update in post_data[
                        "update_info"
                    ].items():
                        windows_version_obj, _ = WindowsVersion.objects.get_or_create(
                        ...
                        for a, b in windows_update.items():
                            if "updateInfo" in b.keys():
                                ...
                                (
                                    windows_update_obj,
                                    _,
                                ) = WindowsUpdate.objects.get_or_create(
                                    name=a,
                                    release_date=release_date,
                ...
                dll_instance_object.windows_updates.set(
                    instance_windows_updates,
                )
        ...
        for dll_name in dll_names:
            ...
            if len(ordered_dlls) >= 2:
                ...
                first_dll_dict["windows_updates"] = [
                    model_to_dict(
                        t,
                    )
                    for t in first_dll.windows_updates.all()
                ]
                last_dll_dict["windows_updates"] = [
                    model_to_dict(
                        t,
                    )
                    for t in last_dll.windows_updates.all()
                ]
                ...
                versions_dict = [
                    {
                        ...
                        "insider": True
                        if (version.windows_updates.first().name[0:2] == "IP")
                        else False,
                    }
                ...
                response_json[dll_name] = {
                    ...
                    "windows_versions": DLL.objects.get(
                        name=dll_name,
...
def list_dlls_by_name(request, dll_name):
    ...
    for i in data:
        i["function_count"] = function_count = Function.objects.filter(
            # could be any of these dll instances
            dll_instance=queryset[counter],
        ).count()
        i["insider"] = (
            True
            if (
                DLLInstance.objects.get(
                    pk=i["id"],
                )
                .windows_updates.first()
                .name[0:2]
                == "IP"
            )
            else False
        )
        i["windows_updates"] = list(
            queryset[counter].windows_updates.values(
                "id",
                "name",
                "release_date",
...
def functions(request):
    queryset = Function.objects.values(
        "dll_instance",
        "function_name",
        "function_c",
        "function_c_hash",
